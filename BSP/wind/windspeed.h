...
#define gao_SPEED_LOW_RPM       80         /* gao调试的时候传为0转 */
#define gao_SPEED_HIGH_RPM      220         /* gao调试的时候220转速 */
...
void WindSpeed_Init(void);                                /* 初始化风速算法数据结构 */
void WindSpeed_Update(u8 temp, u8 humidity, float gas);  /* 更新传感器数据并计算风速 */
float WindSpeed_GetPWM(void);                             /* 获取计算得到的PWM占空比 */
u16 WindSpeed_GetPWMCCR(u16 maxCompare);                /* 获取PWM占空比对应的CCR值 */
u8 WindSpeed_IsCookingEvent(void);                        /* 判断是否为Cooking Event */
WindSpeed_t* WindSpeed_GetData(void);                        /* 获取当前风速算法数据结构指针 */
u16 WindSpeed_GetTargetRPM(u8 level);                        /* 根据档位获取目标转速 */

u16 gao_WindSpeed_GetTargetRPM(u8 level);                        /* gao用来调试PID函数 */
